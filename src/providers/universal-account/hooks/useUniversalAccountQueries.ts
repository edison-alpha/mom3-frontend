...
export function useRefreshAccount(ownerAddress?: string) {
  ...
  return async () => {
    // A retry must not prepare a quote against the previous balance snapshot.
    // Invalidate first, then await the active refetch before the caller creates
    // another UserOperation.
    await queryClient.invalidateQueries({
      queryKey: universalAccountQueryKeys.snapshot(ownerAddress),
      refetchType: "none",
    });
